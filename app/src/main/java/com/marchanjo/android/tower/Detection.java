package com.marchanjo.android.tower;
...
public class Detection {
    ...
    private void traceLinesPlus(Mat dest, Mat lines)//lines é uma matriz mas contem somente retas vindas da transformada de Hough
    {
        ...
        for (int i = 0; i < lines.rows(); i++) {
            ...
            if ((angle[i] >= 89) && (angle[i] <= 91)) { angleRaw[i] = 90; red=255; green=0; blue=128; }
            ...
            if(!((red==0)&&(green==255)&&(blue==0))) //se não for verde (cor default), imprime. Isto é, somente imprime retas com ângulos identificados
                Imgproc.line(dest, new Point(l[0], l[1]), new Point(l[2], l[3]), new Scalar(red,green,blue), 1, Imgproc.LINE_AA, 0);
        }
        /*
    //Determina pontos de um bounding box básico (basiados em retas verticais)
       ...
       /* for(int i = 0; i<class45IdCounter; i++ ){line45[i] = new Feature();}
        ...
        //identifyClusters(dest, line90, class90IdCounter, new Scalar(128, 0, 255));
...
public class Detection {
    ...
    public Mat preProcessing(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        //mRgba = inputFrame.rgba();
        mOut = new Mat(inputFrame.rgba().height(),inputFrame.rgba().width(), CvType.CV_8UC4);
        mIntermediate = new Mat(inputFrame.rgba().height(),inputFrame.rgba().width(), CvType.CV_8UC4);
        probHoughLines = new Mat(inputFrame.rgba().height(),inputFrame.rgba().width(), CvType.CV_8UC4);
        //mIntermediateMat = inputFrame.rgba();
        ...
        Imgproc.blur(inputFrame.rgba(), mIntermediate, new Size(3, 3));
        Imgproc.Canny(mIntermediate, mOut, 80, 100);

        Mat linesP = new Mat(); // will hold the results of the detection
        Imgproc.HoughLinesP(mOut, linesP, 1, Math.PI/180, 50, 50, 10); // runs the actual detection //três últimos números 54,12,9
    ...
    private void traceLinesPlus(Mat dest,Mat lines)
    {
       double temp;
       double[] angle = new double[lines.rows()];
       int[] angleRaw = new int[lines.rows()];
      ...
      // ArrayList<Integer> angleRaw = new ArrayList<Integer>(lines.rows());
        ...
        for (int x = 0; x < lines.rows(); x++) {
            ...
            if (temp < 0) angle[x] = temp + 180; //substitui valores como -135° por 45°
            else angle[x] = temp;
            Log.i("Angle B=", Double.toString(angle[x]));

            //de 179º a 181º  e 0º a 1º (não tem não tem negativo)
            if ((angle[x] >= 179) && (angle[x] <= 181)) { angleRaw[x] = 180; red=0; green=0; blue=255;}//Azul
            if ((angle[x] >= 0) && (angle[x] <= 1)) { angleRaw[x] = 180; red=0; green=0; blue=255;}//Azul

            //85°, 90°, 95° linhas verticais - torre
            if ((angle[x] >= 80) && (angle[x] < 89)) { angleRaw[x] = 85; red=255; green=0; blue=255; }//Left leg
            if ((angle[x] >= 89) && (angle[x] <= 91)) { angleRaw[x] = 90; red=255; green=200; blue=255; }
            if ((angle[x] > 91) && (angle[x] <= 100)) { angleRaw[x] = 95; red=255; green=0; blue=0; }//Right leg

            //45° e 135° linha diagonais - treliça (desloquei o range para 125 a 155 ficou melhor), antes tinha if ((angle[i] >= -155) && (angle[i] <= -125))
            if ((angle[x] >= 125) && (angle[x] <= 155)) { angleRaw[x] = 135; red=255; green=127; blue=0;  }//Laranja  useColorLine = new Scalar(255, 127, 0);
            if ((angle[x] >= 25) && (angle[x] <= 55)) { angleRaw[x] = 45; red=255; green=255; blue=0;   }//Amarelo useColorLine = new Scalar(255, 255, 0);

            if(!((red==0)&&(green==255)&&(blue==0))) //se não for verde (cor default), imprime. Isto é, somente imprime retas com ângulos identificados
                Imgproc.line(probHoughLines, new Point(l[0], l[1]), new Point(l[2], l[3]), new Scalar(red,green,blue), 1, Imgproc.LINE_AA, 0);
        }
    }
...
//    {
//        double temp;
//        double* angle = new double[lines.size()];
//        int* angleRaw = new int[lines.size()];
//        int* classifier = new int[lines.size()];
//        //double* hyp = new double[lines.size()];
//        //int x0=0, y0=0, width=dest.cols, height=dest.rows;
//        int x0 = dest.cols, y0 = dest.rows;//começa na direita abaixo para ser deslocado para a esquerda superior
//        int x1 = 0, y1 = 0;//começa na esquerda superior para ser deslocado para a direita abaixo
//        roi detectedRoi;
//        Vec4i li, lj;
//        twoPoints rect;
//
//        Scalar colorRect= Scalar(0, 0, 255);
//        //Calcula angle e define angleRaw
//        for (int x = 0; x < lines.rows(); x++)
//        {
//            classifier[i] = 0;//não classificado
//            Vec4i l = lines[i];
//
//            temp= atan2((double)l[3] - l[1], (double)l[0] - l[2]) * 180 / 3.14159265; //angle = atan2 (y,x) * 180 / PI; usei l[3]-l[1] porque na tela o y cresce para baixo, e no gráfico cartesiano para cima. De qualquer forma afoi deste jeito que os ângulos ficaram coerente com a apresentação na tela
//
//            if (temp < 0) angle[i] = temp + 180; //substitui valores como -135° por 45°
//            else angle[i] = temp;
//            angleRaw[i] = 361;//361 é o valor de inicialização, indica não carregado como valor básico:  45°, 85°, 90°, 95°, 135°, 180° (equivale a 0°)
//            //hyp[i] = sqrt(square(l[1] - l[3]) + square(l[0] - l[2]));//hyp = sqrt(square(y1 - y2) + square(x1 - x2));
//
//            Scalar useColorLine=color;
//            //180° linhas horizontais
//            if ((angle[i] >= 179) && (angle[i] <= 181)) { angleRaw[i] = 180; useColorLine = Scalar(255, 0, 0);}//Azul
//
//            //85°, 90°, 95° linhas verticais - torre
//            if ((angle[i] >= 80) && (angle[i] < 89)) { angleRaw[i] = 85; useColorLine = Scalar(255, 0, 255); }//Left leg
//            if ((angle[i] >= 89) && (angle[i] <= 91)) { angleRaw[i] = 90; useColorLine = Scalar(255, 200, 255); }
//            if ((angle[i] > 91) && (angle[i] <= 100)) { angleRaw[i] = 95; useColorLine = Scalar(0, 0, 255); }//Right leg
//
//            //45° e 135° linha diagonais - treliça (desloquei o range para 125 a 155 ficou melhor), antes tinha if ((angle[i] >= -155) && (angle[i] <= -125))
//            if ((angle[i] >= 125) && (angle[i] <= 155)) { angleRaw[i] = 135; useColorLine = Scalar(0, 255, 255);  }//Amarelo
//            if ((angle[i] >= 25) && (angle[i] <= 55)) { angleRaw[i] = 45; useColorLine = Scalar(0, 255, 255);  }//Amarelo
//
//            line(dest, Point(l[0], l[1]), Point(l[2], l[3]), useColorLine, 1, LINE_AA);//255,0,0 Azul B,G,R
//            //printf("Angle=%1.2f\t\t\n",angle[i]);
//        }
//
//        for (int x = 0; x < lines.rows(); x++) {
